...
2. Установить nodejs 19.9.0 (с этой версией запускается точно) на компьютер [следуя инструкции](https://nodejs.org/ru/download/);
...
Для этого на п.1 "Запуск BACKEND" вместо запуска всего `docker-compose.yml` следует запустить
только `docker-compose.yml -> main-db`, а само приложение через main метод.
